...
interface DynamicTableProps {
  ...
  onRowSelect?: (data: { source_id?: string; target_id?: string; login?: string; chemical_formula?: string; main_percent?: string }) => void;
}

const DynamicTableOperations: React.FC<DynamicTableProps> = ({ headers = [], data = [], onRowSelect }) => {
  ...
  const handleRowClick = (row: { [key: string]: string | number | null }, index: number) => {
    ...
    if (onRowSelect) {
      if (row.login) {
        onRowSelect({ login: row.login.toString() });
      } else if (row.chemical_formula) {
        onRowSelect({ chemical_formula: row.chemical_formula.toString() });
      } else if (row.main_percent) {
        onRowSelect({ main_percent: row.main_percent.toString() });
      } else if (row.source_ids) {
        onRowSelect({
          source_id: row.source_ids.toString(),
          target_id: row.target_id?.toString() || "", // Передаем target_id, если он есть, иначе пустая строка
        });
      }
    }
  };
...
export default DynamicTableOperations;